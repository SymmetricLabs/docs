...
<CodeGroup>
```bash cURL
curl -X GET "https://api.heytessa.ai/v1/get_workflow_status/your_run_id" \
  -H "Authorization: Bearer $TESSERACT_API_KEY"
```
...
def wait_for_completion(run_id, api_key):
    headers = {'Authorization': f'Bearer {api_key}'}
    
    while True:
        response = requests.get(
            f'https://api.heytessa.ai/v1/get_workflow_status/{run_id}',
            headers=headers
        )
...
# Usage
final_status = wait_for_completion(run_id, api_key)
...
async function waitForCompletion(runId, apiKey) {
  ...
  while (true) {
    const response = await fetch(
      `https://api.heytessa.ai/v1/get_workflow_status/${runId}`,
      { headers }
    );
...
// Usage
const finalStatus = await waitForCompletion(runId, apiKey);
...
class TesseractAPI:
    ...
    def get_status(self, run_id):
        response = requests.get(f"{self.base_url}/get_workflow_status/{run_id}", headers=self.headers)
        return response.json()
...
if workflows:
    ...
    # Wait for completion
    final_status = api.wait_for_completion(run_id)
